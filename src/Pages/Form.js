...
const FormPage = ({ setAgree, agree, setName, name, setSectors, sectors }) => {
  ...
  const handleSave = () => {
    ...
    if (name.length && sectors.length && agree) {
      setAlert(true);
    }
  };
  ...
  return (
    <>
      <div
        class={`${
          alert ? 'block' : 'hidden'
        } bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3 mt-5 ml-10 pl-10`}
        role='alert'
      >
        <p class='font-bold'>Information is saved</p>
        <p class='text-sm'>Click the right side to see the message</p>
      </div>
      <div className='lg:w-3/4 w-[90%] h-auto shadow-md mx-auto block rounded-2xl lg:p-20 sm:p-12 p-6 mt-24'>
        ...
        <div className='mx-auto mt-4'>
          <label
            htmlFor='name'
            className="text-slate-600 text-base font-semibold  after:content-['*'] after:ml-0.5 after:text-red-500"
          >
            Name:
          </label>
          <input
            ...
            className=' border rounded-md mx-auto block h-11 px-5 mt-2  w-full shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 border-slate-400 placeholder-slate-400 focus:bg-white'
            type='text'
            placeholder='Name'
            id='name'
          />
          ...
          <p
            className={`${name ? 'hidden' : 'block'} ${
              name === undefined ? 'hidden' : 'block'
            }  text-red-600 text-semibold text-base mt-2`}
          >
            *Please fill up name
          </p>
        </div>
        ...
        <div className='flex mt-4 items-center '>
          <p className="after:content-['*'] after:ml-0.5 after:text-red-500 text-slate-600 text-base font-semibold">
            Sectors:
          </p>
          <select
            value={sectors}
            onChange={(e) => handleSector(e)}
            className='sm:ml-5 ml-2 border rounded-md lg:w-60 w-full h-11  shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 border-slate-400 placeholder-slate-400 focus:bg-white pl-3 pr-3 text-slate-600 font-semibold'
          >
        ...
        {!sectors && sectors != null && (
          <p className='text-red-600 text-semibold text-base mt-2'>
        ...
        <div className='mt-4 flex items-center gap-2'>
          ...
          />
          <div
            className={`w-5 h-5 rounded-[3px] border border-blue-600 flex justify-center items-center ${
              agree ? 'bg-blue-500' : 'bg-transparent'
            }`}
          >
            <span className={`${agree ? 'text-white' : 'text-black hidden'} `}>
              ✓
            </span>
          </div>
          <label
            role='button'
            className='text-base text-slate-800 font-semibold pl-10 agreeText'
            htmlFor='agree'
          >
            Agree to terms
          </label>
        ...
        <p
          className={`${agree ? 'hidden' : 'block'} ${
            agree === null ? 'hidden' : 'block'
          }  text-red-600 text-semibold text-base mt-2`}
        >
        ...
        <div className='flex sm:flex-row flex-col justify-between items-center mt-4'>
          <button
            onClick={handleSave}
            className='px-6 py-2 mt-4  border border-slate-800 font-bold rounded hover:bg-slate-800 hover:text-white'
          >
            Save
          </button>
          ...
          {alert && (
            <p className='mt-4 sm:mt-0'>
              Result saved{' '}
              <Link to='/result'>
                <span className='underline hover:text-blue-300 font-semibold'>
